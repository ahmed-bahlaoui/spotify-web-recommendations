  ...
  >
    ...
    <div className="p-4">
      <div className="pointer-events-none z-10 flex transform-gpu flex-col gap-1 transition-all duration-300 lg:group-hover:-translate-y-10">
        <Icon className="h-7 w-7 origin-left transform-gpu text-white transition-all duration-300 ease-in-out group-hover:scale-75" />
        <h3 className="text-xl font-semibold text-white">{name}</h3>
        <p className="max-w-lg text-neutral-200 text-sm">{description}</p>
      </div>
      ...
      >
        <Button
          ...
          asChild
          size="sm"
          className="pointer-events-auto p-0 text-neutral-300"
        >
          <a href={href}>
            {cta}
